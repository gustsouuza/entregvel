...
export const PublicSite: React.FC<PublicSiteProps> = ({ isPreviewMode = false }) => {
  ...
  // Guarda quais convidados o usuário já clicou "Confirmar/Recusar" nesta sessão,
  // pra o polling de dados não apagar uma escolha em andamento (mesmo bug/correção
  // aplicada no public-site/src/components/PublicSite.tsx).
  ...
  // Reseta o controle de "tocados" só quando o convite muda de fato (PIN diferente).
  useEffect(() => {
    touchedGuestIdsRef.current = new Set();
  }, [matchedInvitation?.id]);

  // Sincroniza a seleção de presença sem apagar o que o usuário já marcou nesta sessão.
  useEffect(() => {
  ...
  const handleGiftSubmit = (e: React.FormEvent) => {
    ...
    // No modo preview (aba Design & Visual), só simula a tela de sucesso —
    // não grava nada de verdade, pra não gerar contribuição fake no painel real.
    if (!isPreviewMode) {
      addContribution({
  ...
  const handleRsvpSubmit = (e: React.FormEvent) => {
    ...
    if (matchedInvitation) {
      ...
      // No modo preview (aba Design & Visual), não grava nada de verdade —
      // só mostra a tela de sucesso pra visualização, sem tocar no convite real.
      if (!isPreviewMode) {
        const updatedInvitations = (weddingData.invitations || []).map(inv => {
          if (inv.id === matchedInvitation.id) {
            return {
              ...inv,
              guests: updatedGuests,
              phone: rsvpPhone || inv.phone,
              notes: inv.notes ? inv.notes + (rsvpMsg ? ` | Recado: ${rsvpMsg}` : '') : (rsvpMsg ? `Recado: ${rsvpMsg}` : '')
            };
          }
          return inv;
        });
      ...
      const result = isPreviewMode
        ? { name: rsvpName, phone: rsvpPhone, id: `preview-${Date.now()}`, date: new Date().toISOString(), code: `PREVIEW-${Math.floor(10 + Math.random() * 90)}`, attending: rsvpAttending, companions: rsvpAttending ? rsvpCompanions : 0 }
        : submitRSVP({
            name: rsvpName,
            phone: rsvpPhone,
  ...
  const handleGuestMsgSubmit = (e: React.FormEvent) => {
    ...
    // No modo preview, não grava recado real no mural.
    if (!isPreviewMode) {
      submitMessage({
        author: guestName,
        content: guestMsg,
  ...
  return (
    ...
    >
      ...
      {activePage === 'inicio' && (
        ...
        >
          {/* Hero Section */}
          <section className="relative min-h-[90vh] md:min-h-screen flex items-center justify-center text-center px-6 z-10 overflow-hidden">
        {/* Immersive Classical Landscape Background Image */}
        <div className="absolute inset-0 z-0">
          <img 
            src="/assets/images/hero_landscape.jpg" 
            alt="Luciana & Nilton Cover" 
            className="w-full h-full object-cover select-none scale-100 filter brightness-[0.6] transition-all duration-1000"
          />
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-white">
          ...
          <motion.h1 
            ...
            className="font-cinzel text-3xl md:text-6xl tracking-[0.1em] font-light mb-6 text-gray-300 drop-shadow-md select-none"
          >
            LUCIANA & NILTON
          </motion.h1>
            ...
            {gallery.map((item, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`${gridSpanClass} relative rounded-xl overflow-hidden cursor-pointer group shadow-md hover:shadow-xl transition-all duration-300 border border-[#8C7853]/5`}
                >
                  <img 
                    src={item.url} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 border border-[#8C7853]/15 rounded-2xl overflow-hidden shadow-xl">
              ...
              <div className="lg:col-span-5 bg-[#FDFBF7] p-8 md:p-12 flex flex-col justify-center border-t lg:border-t-0 lg:border-l border-[#8C7853]/10">
                ...
                <p className="text-sm text-[#2C2520]/70 leading-relaxed font-light mb-8">
                  Nossa cerimônia terá início pontualmente às 17:00. Recomendamos chegar com 30 minutos de antecedência para desfrutar deste momento tão especial em nossas vidas.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                  ...
                  {matchedInvitation ? (
                    <form onSubmit={handleRsvpSubmit} className="space-y-6">
                      ...
                      <button
                        type="submit"
                        className="w-full bg-[#2C2520] hover:bg-[#1A1613] text-white text-xs font-bold uppercase tracking-widest py-4 rounded-md transition-colors shadow-xs text-center"
                      >
                        Salvar Confirmações
                      </button>
                    </form>
                  ...
                  {/* RSVP Display Results or Ticket */}
                    ...
                    <AnimatePresence mode="wait">
                      {rsvpSuccessData ? (
                        <motion.div
                          ...
                          className="bg-[#8C7853]/10 border-2 border-dashed border-[#8C7853]/40 p-6 rounded-xl text-center flex flex-col items-center"
                        >
            ...
            {giftStep === 'details' && (
              <div className="p-6 space-y-6">
                ...
                <div className="text-center">
                  <span className="text-[10px] text-[#2C2520]/50 italic block mb-4">"A contribuição pode ser de qualquer valor."</span>
                  <button
